...
int settings(string subcmd) {
    if (!squidlib.general.ifstate()) return IFSTATES_FALSE;
...
int runfile(string subcmd) {
    ...
    if (rf) {
        while (!rf.eof()) {
            getline(rf, temp);
            if (squidlib.command.run_command(squidlib.command.compile_quote(temp)) == EXIT_MAIN)
                return EXIT_MAIN;
        }
    }
    else {
...
void regist_command(void) { //注册命令
    ...
    squidlib.regcmd("settings", settings);
...
int main(int argc, char *argv[])
{
    ...
    */  //:thonk:
    ...
    while(1)
    {
        cout << ">>>";
        getline(cin,inp_com);
        inp_com = squidlib.command.compile_quote(inp_com);
        if (squidlib.command.run_command(inp_com) == EXIT_MAIN)
            return 0;
    }
}
